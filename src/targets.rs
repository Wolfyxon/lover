...
use crate::console::{
    exit_err, print_note, print_significant, print_step, print_step_verbose, print_success,
    print_warn, CommandLineSettings,
};
use crate::deps;
    ...
    match ImageReader::new(reader).with_guessed_format() {
        Ok(img_reader) => match img_reader.decode() {
            Ok(mut img) => {
                ...
                args.append(&mut vec![
                    "--set-icon".to_string(),
                    icon_out_path.to_str().unwrap().to_string(),
                ]);
            }
...
pub fn build_windows_zip(arch: Arch) {
    ...
    actions::append_file(
        love.as_path(),
        &exe_src,
        "Embedding game into the LOVE executable",
    );
    ...
    fn check_if_pre_targets_exist() {
        ...
        for target in targets {
            for prev in target.previous {
                assert!(
                    get_target(prev).is_some(),
                    "Previous target '{}' not found for '{}'",
                    prev,
                    target.name
                );
            }
    ...
    fn check_target_deps() {
        ...
        for target in targets {
            ...
            for dep in target.optional {
                assert!(
                    deps::get_dep(dep).is_some(),
                    "Optinal dependency '{}' not found for target '{}'",
                    dep,
                    target.name
                );
            }
