...
use crate::{
    config,
    console::{
        self, confirm_or_exit, exit_err, print_err, print_note, print_step, print_success,
        print_warn, ProgressBar,
    },
    http::{self, Downloadable},
};
...
impl<'a> Dependency<'a> {
    ...
    pub fn get_instance(&'a self) -> DependencyInstance<'a> {
        match &self.mode {
            RepoDownload::LatestRelease(pattern) => {
                DependencyInstance::LatestRelease(ReleaseDependency {
                    base: &self,
                    pattern: &pattern,
                })
            }

            RepoDownload::Source(branch) => DependencyInstance::Source(SourceDependency {
                base: &self,
                branch: &branch,
            }),
        }
    ...
    pub fn fetch_download_url(&self) -> String {
        match &self.get_instance() {
            DependencyInstance::LatestRelease(dep) => dep.fetch_asset().browser_download_url,
            DependencyInstance::Source(dep) => dep.get_download_url(),
        }
    }
...
impl<'a> ReleaseDependency<'a> {
    ...
    pub fn get_asset_from_release(&self, release: &GitHubRelease) -> GithubReleaseAsset {
        ...
        if asset_res.is_none() {
            exit_err(format!(
                "No file matches pattern '{}' in release. This is a bug!",
                &self.pattern
            ));
        }
...
impl<'a> SourceDependency<'a> {
    pub fn get_download_url(&self) -> String {
        format!(
            ...
            self.base.repo_owner, self.base.repo, self.branch
        )
    }
}
...
pub fn get_dep<'a>(name: impl Into<String>) -> Option<Dependency<'a>> {
...
pub fn get_deps_by_strings<'a>(names: Vec<impl Into<String>>) -> Vec<Dependency<'a>> {
    ...
    for name in names {
        res.push(get_dep_or_crash(name));
    }
...
pub fn install(names: Vec<String>) {
    ...
    fetch_bar.set_prefix(format!(
        "{} Fetching dependencies",
        console::get_step_prefix()
    ));
...
pub fn create_dir() {
    ...
    if !dir.exists() {
        ...
        if err.is_err() {
            exit_err(format!(
                "Can't create dependency directory at '{}': {}",
                &dir.to_str().unwrap(),
                err.err().unwrap()
            ));
        }
            ...
            match dep.mode {
                RepoDownload::LatestRelease(pattern) => {
                    Regex::new(pattern).unwrap_or_else(|err| {
                        panic!(
                            "Invalid regex for dep: '{}': {}: {}",
                            dep.name, err, pattern
                        );
                    });
                }
                _ => (),
