...
fn main() {
    ...
    ansi_term::enable_ansi_support().unwrap_or_else(|err_code| {
        print_warn(format!(
            "Failed to enable ANSI color codes. Error code: {}",
            err_code
        ));
    });
...
fn get_commands<'a>() -> Vec<Command<'a>> {
    vec![
        Command {
            ...
            function: cmd_help,
            args: vec![CommandArg::opt("command", "Command to check the usage of")],
            flags: vec![],
        },
        Command {
        ...
        Command {
            ...
            function: cmd_run,
            args: vec![CommandArg::opt(
                "arguments...",
                "Arguments handled by your game.",
            )],
            flags: vec![
                ...
                CommandFlag::new_only_full(
                    "no-prime",
                    "Doesn't run the game on the dedicated GPU if configured",
                ),
            ],
        },
        Command {
        ...
        Command {
            ...
            function: cmd_target,
            args: vec![CommandArg::opt("target", "Name of the target to check.")],
            flags: vec![],
        },
        Command {
            alias: "dep".to_string(),
        ...
        Command {
            ...
            args: vec![CommandArg::req(
                "dependencies...",
                "Names of the dependencies to install.",
            )],
            flags: vec![],
        },
        Command {
        ...
        Command {
            ...
            function: cmd_fetch,
            args: vec![CommandArg::req("name", "Name of the dependency.")],
            flags: vec![],
        },
        Command {
            alias: "env".to_string(),
            ...
            flags: vec![CommandFlag::new_only_full(
                "no-hint",
                "Removes the 'Use os.getenv(name)...' hint under the list",
            )],
        },
        Command {
            alias: "module".to_string(),
            description:
                "Shows the extra code injected into your game when building. Mostly for testing"
                    .to_string(),
            function: cmd_module,
            args: vec![],
            flags: vec![],
...
fn cmd_run(_command: &Command) {
    ...
    if main_script_parent_res.is_some() {
        ...
        if !files::compare_paths(main_script_parent, &main) {
            let main_script_str = files::skip_path_string(&main_script_path, &root);
            let main_script_parent_str = files::skip_path_string(main_script_parent, &root);
            let mut main_str = files::skip_path_string(&main, &root);
            ...
            eprintln!("Main directory: {}", main_str);
...
fn cmd_parse(_command: &Command) {
    let src = project_config::get().paths.get_main_dir();
...
fn cmd_build(command: &Command) {
    ...
    files::create_dir(&project_conf.paths.get_temp_dir());
    ...
    for target in &targets {
        if already_built.contains(&target.name) {
            continue;
        }
        already_built.push(target.name);

        for dep_name in &target.previous {
            if already_built.contains(dep_name) {
                continue;
            }
...
fn cmd_clean(_command: &Command) {
    let build = &project_config::get().paths.get_build_dir();
    ...
    match command.get_arg("target") {
        ...
        None => {
            ...
            for target in get_targets() {
                println!(
                    "- {}: {}",
                    Style::new().fg(Green).paint(target.name),
                    target.description
                )
            }
